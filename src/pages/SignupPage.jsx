  ...
  return (
    <>
      ...
      <section className="mx-auto px-4 sm:px-6 py-10" style={{ width: "1240px" }}>
        {/* Reusable Header */}
        <Header text="Your Cart" size="medium" className="text-center mb-6" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
          ...
          <div className="flex-1 border border-neutral-200 rounded-md bg-white">
            {cartItems.map((item, idx) => (
              <div
                key={item.id}
                className={`flex items-center justify-between px-5 py-4 ${idx !== cartItems.length - 1 ? "border-b border-neutral-200" : ""}`}
              >
                {/* Left */}
                <div className="flex items-center gap-4">
                  <img src={item.image} alt={item.name} className="w-[36px] h-[64px] object-contain" />
                  <div className="leading-tight">
                    <div className="font-semibold text-neutral-800 text-[13px]">{item.name}</div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {item.discountPrice && <span className="text-[12px] font-semibold text-black-600">${item.discountPrice}</span>}
                      {item.price && <span className="text-[11px] text-neutral-500 line-through">${item.price}</span>}
                ...
                <div className="flex flex-col items-end gap-2">
                  <button aria-label="Remove" className="w-7 h-7 grid place-items-center text-neutral-500">
                    <img src="/trash.png" alt="Delete" className="w-4 h-4 object-contain" />
                  </button>
          ...
          <div className="w-full border border-neutral-200 rounded-md p-5 lg:p-6 h-fit">
            <div className="font-semibold text-[14px] text-neutral-800 mb-4">Order Summary</div>
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t pt-2 flex justify-between text-neutral-800"><span>Total</span><span className="text-neutral-900 font-semibold">${total.toFixed(2)}</span></div>
            </div>
            ...
            <div className="flex items-center gap-2 mt-5">
              <div className="relative w-full">
                ...
                <input type="text" placeholder="Add promo code" className="w-full pl-10 pr-4 py-2 text-sm bg-gray-100 text-gray-700 placeholder-gray-400 rounded-full focus:outline-none" />
              </div>
              <button className="px-5 py-2 border border-purple-900 text-purple-900 rounded-full font-semibold text-xs uppercase">Apply</button>
            </div>
